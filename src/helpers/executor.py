...
class Executor:
    ...
    def run(self) -> dict:
        ...
        Returns:
            all_results(dict): structured results with query names as keys
            and dictionaries containing query metadata and results as values.
                Format: {
                    'query n': {
                        'name': str,
                        'task': str,
        ...
        try:
            ...
            all_results["query 1"] = {
                "name": "query 1",
                "task": """Select all location_names that don’t
                   have any sublocations. (self join).""",
                "results": result_1,
            }
            ...
            result_2 = self.db.execute_query(
                ...
                COALESCE(
                    ...
                    root.location_name)
                FROM locations root
                LEFT JOIN locations lev2
                ON lev2.parent_location_id=root.location_id
                LEFT JOIN locations lev3
                ON lev3.parent_location_id=lev2.location_id
                GROUP BY root.location_name,
                    lev2.location_name,
                    lev3.location_name,
                    root.location_id
            ...
            all_results["query 3"] = {
                "name": "query 3",
                "task": """Find all event_id’s for 'Smart Lamp' devices
                    where the new_status was 'on' and the brightness
                    was greater than 80.""",
                "results": result_3,
            }
            ...
            result_4 = self.db.execute_query(
                """SELECT avg((e.details ->> 'brightness')::int) AS brightness,
                    l.location_name
                FROM events e
                JOIN devices d ON d.device_id = e.device_id
            ...
            all_results["query 4"] = {
                "name": "query 4",
                "task": """Calculate the average brightness for all
                'Smart Lamp' 'on' events, grouped by location_name.""",
                "results": result_4,
            }
            ...
            all_results["query 5"] = {
                "name": "query 5",
                "task": """List location_names that
                    have any device that detected a leak.""",
                "results": result_5,
            }
            ...
            all_results["query 6"] = {
                "name": "query 6",
                "task": """Select all location_names and its device_names
                    that don’t have any events.""",
                "results": result_6,
            }
            ...
            all_results["query 7"] = {
                "name": "query 7",
                "task": """Find 3 location_names with the highest amount
                    of 'Smart Lamp' devices, and its amount.""",
                "results": result_7,
            }
            ...
            result_8 = self.db.execute_query(
                """SELECT l.location_name,
                    avg((e.details ->> 'temperature')::float) AS average
                FROM locations l
                JOIN devices d ON d.location_id = l.location_id
                JOIN events e ON d.device_id = e.device_id
            ...
            all_results["query 8"] = {
                "name": "query 8",
                "task": """Calculate the average temperature for each
                    location where alert = 'high_temp'.""",
                "results": result_8,
            }
            ...
            result_9 = self.db.execute_query(
                """SELECT l.location_name,
                    d.device_type,
                    sum((e.details -> 'person_count')::int)
                FROM locations l
                JOIN devices d ON d.location_id = l.location_id
                JOIN events e ON d.device_id = e.device_id
            ...
            all_results["query 9"] = {
                "name": "query 9",
                "task": """Retrieve the total number of people (person_count)
                    for each location and device type, excluding entries
                    with zero values.""",
                "results": result_9,
            }
            ...
            all_results["query 10"] = {
                "name": "query 10",
                "task": """Retrieve the device_id of devices for which
                the very first recorded event (by timestamp) had a
                status of 'off'.""",
                "results": result_10,
            }
            logger.info(
                f"Query 10 executed, {len(result_10)} results returned"
            )
